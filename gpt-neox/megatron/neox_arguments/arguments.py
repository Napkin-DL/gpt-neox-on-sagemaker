...
#         ), f"User script could not be found: {args_parsed.user_script}"
    ...
    def consume_neox_args(cls, overwrite_values=None):
        ...
        ### Revised for SageMaker
        sm_overwrite = True                                           ## add flag for SageMaker
        if args_parsed.megatron_config is not None:
            megatron_config = json.loads(args_parsed.megatron_config)
        else:
            megatron_config = json.loads(overwrite_values)         
            sm_overwrite = False
            
        if overwrite_values is not None and sm_overwrite:              ## add flag for SageMaker
    ...
    def configure_distributed_args(self):
        ...
        ### Revised for SageMaker
        self.update_value("local_rank", int(os.getenv("OMPI_COMM_WORLD_LOCAL_RANK", "0")))
        self.update_value("rank", int(os.getenv("OMPI_COMM_WORLD_RANK", "0")))
        self.update_value("world_size", int(os.getenv("OMPI_COMM_WORLD_SIZE", "1")))
...
#         self.update_value("world_size", int(os.getenv("WORLD_SIZE", "1")))
    ...
    def calculate_derived(self):
        ...
        ### Added for SageMaker
        global_num_gpus = None
        try:
            self.global_num_gpus = int(os.environ['OMPI_COMM_WORLD_SIZE'])
        except:
            pass
        ### Added for SageMaker

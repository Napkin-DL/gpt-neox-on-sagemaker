...
def sagemaker_init(dist_backend):
    print("########### SageMaker INIT #########")
    import torch, os


    os.environ['RANK']= str(os.environ['OMPI_COMM_WORLD_RANK'])
    os.environ['LOCAL_RANK']= str(os.environ['OMPI_COMM_WORLD_LOCAL_RANK'])
    os.environ['WORLD_SIZE']= str(os.environ['OMPI_COMM_WORLD_SIZE'])
...
def _initialize_distributed(neox_args):
    ...
    if torch.distributed.is_initialized():
        ...
        sagemaker_init(dist_backend=neox_args.distributed_backend)
